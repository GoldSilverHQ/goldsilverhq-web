import { createFileRoute, Link } from "@tanstack/react-router";
import { Breadcrumb } from "@/components/Article";
import { SiteShell } from "@/components/SiteShell";
import { HISTORY_PEOPLE, HISTORY_VIP_DESCRIPTION } from "@/lib/content/history-people";
import { seoTitle } from "@/lib/content/map";
import { pageShareMeta } from "@/lib/seo/share-meta";

export const Route = createFileRoute("/history/vip/")({
  head: () => ({
    meta: pageShareMeta({
      title: seoTitle("Notable people"),
      description: HISTORY_VIP_DESCRIPTION,
      path: "/history/vip",
    }),
  }),
  component: VipIndex,
});

function VipIndex() {
  return (
    <SiteShell>
      <div className="mx-auto max-w-6xl px-4 py-12">
        <Breadcrumb
          items={[
            { href: "/", label: "Home" },
            { href: "/history", label: "History" },
            { label: "People" },
          ]}
        />
        <p className="text-xs font-semibold tracking-[0.14em] text-gold uppercase">History</p>
        <h1 className="mt-2 font-display text-4xl sm:text-5xl">Notable people</h1>
        <p className="mt-3 max-w-2xl text-lg text-muted">{HISTORY_VIP_DESCRIPTION}</p>
        <ol className="mt-10 grid gap-3 sm:grid-cols-2">
          {HISTORY_PEOPLE.map((person) => (
            <li key={person.slug}>
              <Link
                to="/history/vip/$person"
                params={{ person: person.slug }}
                className="flex gap-4 rounded-lg bg-surface px-4 py-4 shadow-[var(--shadow-border)] hover:shadow-[var(--shadow-border-hover)]"
              >
                <span className="min-w-0">
                  <span className="block font-medium">{person.name}</span>
                  <span className="text-sm text-muted">
                    {person.role} · {person.life}
                  </span>
                  <span className="mt-1 block text-sm text-muted">{person.summary}</span>
                </span>
              </Link>
            </li>
          ))}
        </ol>
        <p className="mt-10 text-sm">
          <Link to="/history" className="text-gold hover:text-gold-soft">
            ← Sound Money History
          </Link>
        </p>
      </div>
    </SiteShell>
  );
}
